...
A hands-on project self-hosting an open-weights LLM (`Qwen/Qwen3-4B-Instruct-2507`) on a GKE GPU node via vLLM, exposed through an OpenAI-compatible API, and running an offline batch-inference Kubernetes Job that contends for the same single GPU. A follow-up iteration fronts vLLM with a KServe `InferenceService` to compose the serving stack from the [`kserve/`](../kserve/) project.
...
| Toleration       | Pod spec (vLLM Deployment, batch Job)    | `key: nvidia.com/gpu`, `operator: Exists`, `effect: NoSchedule`        | Lets the pod bypass the taint — does **not** attract it               |
...
Originally built on NVIDIA T4 (`n1-standard-4`); migrated to NVIDIA L4 (`g2-standard-4`) on 2026-09-03. Three independent pressures all pointed the same way:

| Pressure | Detail |
| --- | --- |
| Cloud lifecycle | T4 has a published GCP [end-of-support date of 2027-08-01](https://cloud.google.com/compute/docs/eol/t4-eos). New 3-year CUDs are already blocked. |
| Runtime support | vLLM's Turing (sm_75) support is decaying — FlashInfer was dropped from SM75 backends in v0.24.0, and newer model families ship without SM75 kernels. `FLASH_ATTN` has always required sm_80+. |
| Model ecosystem | T4 is fp16-only. Current small open models are bf16-native; Gemma 3 overflows to NaN under fp16. A T4 silently disqualifies most of the 2026 model crop. |

L4 is Ada (sm_89): bf16-capable, 24 GB instead of 16 GB, FlashAttention-eligible, and not on a sunset path.

Cost is the tradeoff — roughly **$0.70/hr vs $0.35/hr** on-demand. For session-scoped study use (a few hours a week, cluster deleted at session end) that is a few dollars a month, which is the right trade against building on hardware that is aging out of the ecosystem.

**Bonus discovery:** T4 is only offered in `us-west1-a` and `us-west1-b` — *not* `-c`. The three-zone failover design below was quietly broken on T4 (node pool creation would have failed on the unsupported zone) and was never caught because the us-west1 config was never run live. L4 is offered in all three zones, so the design now works as originally intended.

### Two-pool GPU setup + multi-zone failover

| Layer             | Config                                                               | Handles                                          |
| ----------------- | -------------------------------------------------------------------- | ------------------------------------------------ |
| On-demand pool    | `gpu-pool-ondemand`, L4, autoscale 0–1 total                         | Preferred — no preemption, predictable benchmarks |
| Spot pool         | `gpu-pool-spot`, L4 + `--spot`, autoscale 0–1 total                  | Fallback — cheaper, ~30s preempt notice          |
...
| Total-node cap    | Both pools: `--total-max-nodes=1`                                    | Honours global GPU quota (1 GPU)                 |
...
Before deploying anything real (vLLM, batch Jobs), run a minimal CUDA pod that only calls `nvidia-smi`. Four independent things have to work for a GPU workload to schedule, and each can fail silently on a fresh cluster:

1. **Autoscaler scale-up** — the GPU pool is 0 nodes when idle; the pending pod has to trigger provisioning of an L4 node in a zone with capacity.
...
#    ComputeClass gpu-l4 applied)
bash cluster.sh create

# 2. Apply the smoketest pod — selects compute-class gpu-l4 and triggers
#    the on-demand L4 scale-up (Spot only if on-demand is stocked out)
...
The progression should be: **pod `Pending`** → autoscaler `TriggeredScaleUp` event → **new node `NotReady`** (driver installing) → **node `Ready`** with `nvidia.com/gpu: 1` in `Capacity` → **pod `Running`** → `nvidia-smi` prints the GPU.

Reference output below is **from the 2026-04-16 run, which used the pre-migration T4 setup** — kept as a real recorded trace rather than an idealized one. On the current L4 config, expect `nvidia-l4` in the accelerator column, `NVIDIA L4` and `23034 MiB` from `nvidia-smi`, and — because the Custom Compute Class now enforces on-demand-first priority — the `gpu-pool-ondemand` pool rather than `gpu-pool-spot`. That last difference is the point of the CCC; if you still land on Spot with on-demand available, the class is not being applied.
...
Default pool alone costs ~$0.13/hr; an L4 node adds ~$0.70/hr on-demand (~$0.22/hr Spot). Always delete at session end.

## Scripts

| Script              | What it does                                                                                                                                                                                                                                                              |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cluster.sh create` | Creates GKE cluster `vllm-gpu-study` in `us-west1-b` with Gateway API + Workload Identity: 1× `e2-standard-4` default pool + two L4 node pools (`gpu-pool-ondemand` and `gpu-pool-spot`, both `g2-standard-4`, autoscale 0–1 total, zones `b,c,a`, `--location-policy=ANY`), then applies `compute-class.yaml` so pods get on-demand-first priority with Spot fallback |
...
| On-demand GPU pool        | `gpu-pool-ondemand`: 1× `g2-standard-4` + NVIDIA L4 (24 GB), `--total-max-nodes=1` |
| Spot GPU pool             | `gpu-pool-spot`: 1× `g2-standard-4` + NVIDIA L4 (`--spot`), `--total-max-nodes=1` |
| GPU pool zones            | `us-west1-b, us-west1-c, us-west1-a` (with `--location-policy=ANY`)           |
| Global GPU quota          | 1 GPU across all regions → at most one GPU node runs at a time                |
...
- **vLLM image pull time** — multi-GB image, 5–10 min on a cold GPU node. If a pod sits in `ContainerCreating` right after the node provisions, check `kubectl describe pod` for pull progress before assuming a config bug.
- **GPU quota=1** — only one GPU workload at a time. Scale vLLM to 0 before launching a batch Job, scale back to 1 after the Job completes.
- **HuggingFace auth** — Qwen3 is Apache 2.0 and ungated, but gated alternatives (Llama, Gemma variants) need `HF_TOKEN` as an env var sourced from a K8s Secret.
- **`--max-model-len` is not optional on Qwen3.** The model's default context is 262144 tokens; vLLM will try to size a KV cache for it and OOM on load. Set it explicitly (16384 is comfortable on a 24 GB L4).
